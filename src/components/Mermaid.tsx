...
const stripNonMermaidTail = (chart: string): string => {
  const lines = chart.replace(/\r\n/g, '\n').split('\n');
  const cleaned: string[] = [];

  for (const line of lines) {
    const trimmed = line.trim();
    if (/^<system-reminder>/i.test(trimmed)) break;
    cleaned.push(line);
  }
...
const normalizeSingleLineFlowchart = (chart: string): string => {
  const normalized = stripNonMermaidTail(chart);
  if (!/^(graph|flowchart)\s+/i.test(normalized)) {
    return normalized;
  }
  ...
  const headerMatch = normalized.match(/^(graph|flowchart)\s+([A-Z]{2})(?:\s+)?/i);
  if (!headerMatch) {
    return normalized;
  }

  const header = `${headerMatch[1]} ${headerMatch[2]}`;
  let body = normalized.slice(headerMatch[0].length).trim();

  // Drop common accidental trailing commentary after the diagram body.
  body = body.replace(/\s+出现未渲染问题.*$/u, '').trim();

  const statements: string[] = [];
  const edgeRegex = /(\S+(?:\[[^\]]*\]|\([^)]*\)|\{[^}]*\})?)\s*(--(?:\|[^|]*\|)?>|==(?:\|[^|]*\|)?>|-.->|==>)\s*(\S+(?:\[[^\]]*\]|\([^)]*\)|\{[^}]*\})?)/y;

  while (body.length > 0) {
    edgeRegex.lastIndex = 0;
    const match = edgeRegex.exec(body);
    if (!match) {
      return normalized;
    }
...
const sanitizeMermaidChart = (chart: string): string => {
  const normalized = normalizeSingleLineFlowchart(chart).replace(/\r\n/g, '\n').trim();

  if (!/^sequenceDiagram\b/m.test(normalized)) {
    return normalized;
  }
  ...
  const extractParticipantFromArrow = (line: string): string | null => {
    const arrowMatch = line.match(/^\s*([A-Za-z0-9_]+)\s*[-.]+[>x)]+[+-]?\s*([A-Za-z0-9_]+)\s*:/);
    if (!arrowMatch) {
      return null;
    }
    ...
    const activateWithSuffix = /->>\+|-->>\+|->\+|-\)\+|--\)\+/.test(compactLine);
    const deactivateWithSuffix = /->>-|-->>-|->-|-->-|-\)-|--\)-/.test(compactLine);

    if (activateWithSuffix) {
      activeStack.push(target);
      return null;
    }

    if (deactivateWithSuffix) {
      const lastIndex = activeStack.lastIndexOf(target);
      if (lastIndex === -1) {
        return line.replace(/([-.]+[>x)])-/, '$1');
      }
      activeStack.splice(lastIndex, 1);
    }
  ...
  for (const line of lines) {
    ...
    if (/^activate\s+/i.test(trimmed)) {
      const participant = trimmed.split(/\s+/)[1];
      activeStack.push(participant);
      sanitizedLines.push(line);
      continue;
    }
    ...
    if (/^deactivate\s+/i.test(trimmed)) {
      const participant = trimmed.split(/\s+/)[1];
      const lastIndex = activeStack.lastIndexOf(participant);
      if (lastIndex === -1) {
        continue;
      }
      activeStack.splice(lastIndex, 1);
    ...
    const repairedArrow = extractParticipantFromArrow(line);
    sanitizedLines.push(repairedArrow ?? line);
  }

  return sanitizedLines.join('\n');
};

// Initialize mermaid with defaults tuned for Chinese text rendering
mermaid.initialize({
  startOnLoad: true,
  theme: 'neutral',
  securityLevel: 'loose',
  ...
  themeCSS: `
    /* Diagram styles tuned for Chinese text rendering */
  ...
  `,
  fontFamily: '"Noto Sans SC", "PingFang SC", "Microsoft YaHei", "Heiti SC", sans-serif',
  fontSize: 12,
});
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-color)]">
          <div className="font-medium text-[var(--foreground)]">图表查看</div>
  ...
  useEffect(() => {
    ...
    const renderChart = async () => {
      ...
      try {
        ...
        try {
          ({ svg: renderedSvg } = await mermaid.render(idRef.current, sanitizedChart));
        } catch (firstError) {
          const retriedChart = sanitizeMermaidChart(sanitizedChart);
          ({ svg: renderedSvg } = await mermaid.render(idRef.current, retriedChart));
        }
        ...
        if (isMounted) {
          ...
          if (mermaidRef.current) {
            mermaidRef.current.innerHTML = `
              <div class="text-red-500 dark:text-red-400 text-xs mb-1">图表语法错误</div>
              <pre class="text-xs overflow-auto p-2 bg-gray-100 dark:bg-gray-800 rounded">${escapeHtml(chart)}</pre>
            `;
          }
        }
      }
    ...
    return (
      <div className={`flex justify-center items-center p-4 ${className}`}>
        <div className="flex items-center space-x-2">
          ...
          <span className="text-[var(--muted)] text-xs ml-2">正在渲染图表...</span>
        </div>
      </div>
    );
  }
